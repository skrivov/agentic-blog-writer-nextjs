...
async function assessFactuality(state: typeof FactCheckState.State) {
    const structuredLlm = llm.withStructuredOutput(FactualityDecisionSchema, { name: "Factuality Decision" });
...
async function checkFacts(state: typeof FactCheckState.State) {
    ...
    for (const claim of verifiableClaims) {
        ...
        const structuredLlmForClaim = llm.withStructuredOutput(SingleClaimCheckSchema, { name: "Single Fact Check" });
